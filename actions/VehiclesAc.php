...
function createVehicle($conn, $user_id) {
    ...
    if ($foto_vehiculo) {
        $sql = "INSERT INTO vehiculos (id_chofer, placa, color, marca, modelo, anio, capacidad, foto_vehiculo) 
                VALUES ('$user_id', '$placa', '$color', '$marca', '$modelo', '$anio', '$capacidad', '$foto_vehiculo')";
    } else {
        $sql = "INSERT INTO vehiculos (id_chofer, placa, color, marca, modelo, anio, capacidad) 
...
function updateVehicle($conn, $user_id) {
    ...
    if ($foto_vehiculo) {
        $sql = "UPDATE vehiculos SET placa = '$placa', color = '$color', marca = '$marca', 
                ...
                WHERE id = '$vehicle_id' AND id_chofer = '$user_id'";
    } else {
        $sql = "UPDATE vehiculos SET placa = '$placa', color = '$color', marca = '$marca', 
...
function deleteVehicle($conn, $user_id) {
    ...
    $foto_vehiculo = getVehiclePhoto($conn, $vehicle_id);
    
    // Eliminar la foto del servidor si existe
    if (!empty($foto_vehiculo) && file_exists('../' . $foto_vehiculo)) {
        unlink('../' . $foto_vehiculo);
    }
    
    // Eliminar el vehículo de la base de datos
    $sql = "DELETE FROM vehiculos WHERE id = '$vehicle_id' AND id_chofer = '$user_id'";
...
function getVehiclePhoto($conn, $vehicle_id) {
    $sql = "SELECT foto_vehiculo FROM vehiculos WHERE id = '$vehicle_id'";
    $result = mysqli_query($conn, $sql);
